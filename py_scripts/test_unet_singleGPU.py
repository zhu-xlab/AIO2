'''
test script for the Massachusetts dataset:
...
from dataset.data_loading_Building import BuildingDataset, DataSubset
...
def get_args():
    ...
    parser.add_argument('--noise_dir_name', type=str, dest='ndn', default='seg_ns_rm')
...
def evaluate(net, dataloader, device, experiment='gt_labels', save=None):
    ...
    oa_score = 0.
    dice_score = 0.
    pr_score = torch.zeros([net.n_classes+1], device=device) if net.n_classes>1 else 0.
    rc_score = torch.zeros([net.n_classes+1], device=device) if net.n_classes>1 else 0.
    iou_score = torch.zeros([net.n_classes+1], device=device) if net.n_classes>1 else 0.
...
def main():
    ...
    test_dataset = BuildingDataset(args.data_path, noise_dir_name=args.ndn, split=args.split, aug=False)
    ...
    ###### 3 - create model ######
    if args.mt=='unet':
        net = UNet(n_channels=args.n_channels, n_classes=args.n_classes) 
    elif args.mt=='res18':
        net = ResNet18UNet(n_channels=args.n_channels, n_classes=args.n_classes)   
    elif args.mt=='res50':
    ...
    print("Well-trained model and optimizer have been loaded from '{}'\n".format(ckp_path))
