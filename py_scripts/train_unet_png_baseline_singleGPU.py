'''
baseline for the Massachusetts dataset:
...
def get_args():
    ...
    parser.add_argument('--label_dir_name', type=str, dest='ndn', default='seg')
...
def main():
    ...
    if args.experiment == 'ns_labels':
        ndn_split = args.ndn.split('_')
        if ndn_split[-1]!='seg':  # e.g., ndn=ns_seg_rm_5
            args.unet_type += f'_{ndn_split[-2]}_{ndn_split[-1]}'
    args.ndn += f'_{args.mcr}'
    if args.sc_sfs>0: args.unet_type += '_soft'
    if args.opt=='sgd': args.unet_type += f'_{args.opt}'
    # args.job_type = 'baseline' # = 'baseline_ema' if args.ensembling else 'baseline'
    ...
    if args.schd: scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=10, factor=0.5)  # goal: maximize Dice score
    ...
    for epoch in range(last_epoch, args.epochs):
            ...
            if args.cal_tr:
                ...
                if args.experiment == 'ns_labels':
                    tr_accs, btr_dict, tr_gt_accs, btr_gt_dict = \
                        evl.evaluate_batch(pred_masks, true_masks, net.n_classes, 
            ...
            if args.ensembling:
                ...
                if args.experiment == 'ns_labels':
                    tr_accs_mit, btr_dict_mit, tr_gt_accs_mit, btr_gt_dict_mit = \
                        evl.evaluate_batch(pred_mit_masks, true_masks, net.n_classes, 
        ...
        # 1> Validation
        val_metrics = evl.evaluate(net, val_loader, device, experiment=args.experiment)
        ...
        if args.ensembling:
            ...
            # - validation
            val_metrics_mit = evl.evaluate(net_ema_it, val_loader, device, experiment=args.experiment)
            # reframe val_metrics to val_dict
            ...
            if args.esb_ep:
                ...
                # - validation
                val_metrics_mep = evl.evaluate(net_ema_ep, val_loader, device, experiment=args.experiment)
                # reframe val_metrics to val_dict
                ...
                if args.experiment == 'ns_labels':
                    _, btr_dict_mep, _, btr_gt_dict_mep = \
                        evl.evaluate_batch(pred_logits_ema_ep, true_masks, net.n_classes, 
        ...
        if (epoch+1)%args.save_inv==0 or epoch==0:
            ...
            torch.save(save_dict, 
                       args.checkpoints_dir + '/checkpoint_mcr_{}_epoch_{}.pth'.format(args.mcr+1,epoch+1))
