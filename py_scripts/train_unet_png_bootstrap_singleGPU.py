'''
bootstrap for the Massachusetts dataset:
...
def main():
    ...
    args.unet_type = f'{args.mt}_ns_{ndn_split[-2]}_{ndn_split[-1]}_bst'   # e.g., unet_ns_rm_5_bst bst->bootstrap
    ...
    for epoch in range(last_epoch, args.epochs):
        ...
        if args.ema:
            # - validate on noisy labels
            val_metrics_mit = evl.evaluate(net_ema, val_loader, device, experiment='ns_labels')
            # reframe val_metrics to val_dict
            val_dict_mit = evl.add_suffix_to_metric_dict(val_metrics_mit, suffix='mit_v')
            # - validate on gt
            val_gt_metrics_mit = evl.evaluate(net_ema, val_loader, device, experiment='gt_labels')
            # merge val_metrics_gt into val_dict
            val_dict_mit = evl.add_suffix_to_metric_dict(val_gt_metrics_mit, suffix='mit_vg',new_dict=val_dict_mit)
            # - test on gt
            ts_metrics_mit = evl.evaluate(net_ema, test_loader, device, experiment='gt_labels')
            # reframe ts_metrics to ts_dict
            ts_dict_mit = evl.add_suffix_to_metric_dict(ts_metrics_mit, suffix='mit_ts')
            # - test on noisy labels
            ts_metrics_ns_mit = evl.evaluate(net_ema, test_loader, device, experiment='ns_labels')
            # merge val_metrics_gt into val_dict
