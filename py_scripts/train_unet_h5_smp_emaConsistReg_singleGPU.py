'''
EMA Consistency regularization for the Germany dataset:
...
def main():
    ...
    ndn_split = args.nsname[:-3].split('_')
    assert ndn_split[-3]=='ns', "Directory name of noisy labels is invalid!"
    args.unet_type = f'{args.mt}_ns_{ndn_split[-2]}_{ndn_split[-1]}_cst' # e.g., unet_ns_rm_5
    ...
    for epoch in range(last_epoch, args.epochs):
        ...
        for bi, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                if 'd' in args.loss_type:
                    if args.n_classes>1:
                        dice_true = F.one_hot(ns_masks, args.n_classes).permute(0, 3, 1, 2).float()
