'''
Pixel-wise correction for the Germany dataset:
...
from dataset.data_loading_Building_h5 import BuildingDataset, copy_original_labels
...
def main():
    ...
    ndn_split = args.nsname[:-3].split('_')   # e.g., data_all_cities_ns_rm_50
    ...
    os.mkdir(args.cs_dir)
    copy_original_labels(os.path.join(args.data_dir,args.pname),   # partition file
                         os.path.join(args.data_dir,args.nsname),  # noisy label file
                         args.cs_dir, split='train', partition_version=args.mcr)
    ...
    train_dataset = BuildingDataset(args.data_dir, args.dname, args.nsname, 
                                    ...
                                    pcorrect_label_dirname=args.cs_dir, load_origin=True)
    train_dataset_c = BuildingDataset(args.data_dir, args.dname, args.nsname, 
                                      args.pname, args.mcr, split='train', aug=False,
                                      pcorrect_label_dirname=args.cs_dir, load_origin=False)
    ...
    for epoch in range(last_epoch, fepochs):
        ...
        for bi, batch in enumerate(train_loader):
            ...
            if args.cal_tr:
                ...
                tr_accs, btr_dict, tr_gt_accs, btr_gt_dict = \
                    evl.evaluate_batch(pred_masks, ons_masks, args.n_classes, 
            ...
            tr_accs_mit, btr_dict_mit, tr_gt_accs_mit, btr_gt_dict_mit = \
                evl.evaluate_batch(pred_mit_masks, ons_masks, args.n_classes, 
        ...
        if args.esb_ep:
            ...
            _, btr_dict_mep, _, btr_gt_dict_mep = \
                evl.evaluate_batch(pred_mep_masks, ons_masks, args.n_classes, 
        ...
        # save checkpoints
        # Path(args.checkpoints_dir).mkdir(parents=True, exist_ok=True)
        # torch.save(net.state_dict(), args.checkpoints_dir + '/checkpoint_mcr_{}_epoch_{}.pth'.format(args.mcr+1,epoch+1))
